...
function addNewSpellAndAlias(key, spell_name, short_name, alias_command, alias_flags, alias_script, layers, groups, upMessage, downMessage)
    ...
    if (alias_flags == aliasEnabledAndRegex) then
        AddAlias(alias_name, "^" .. alias_command, "cast " .. string.lower(spell_name) .. " at %1", aliasEnabledAndRegex, alias_script)
    else
        AddAlias(alias_name, "^" .. alias_command, "cast " .. string.lower(spell_name), aliasEnabledAndRegex, alias_script)
    end
